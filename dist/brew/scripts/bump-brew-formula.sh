...
Updates Formula/hekatessh.rb in the Homebrew tap repo with release asset sha256
values, commits the change, and opens a pull request with gh.

Set HEKATESSH_BREW_DRY_RUN=1 to update files without committing or opening a PR.
EOF
}
...
version="${tag#v}"
base_url="https://github.com/naoto256/hekatessh/releases/download/${tag}"
tap_repo="naoto256/homebrew-hekatessh"
branch="bump/hekatessh-${version}"
dry_run="${HEKATESSH_BREW_DRY_RUN:-0}"
...
formula="${tap_dir}/Formula/hekatessh.rb"
if [ ! -f "${formula}" ]; then
  echo "missing tap formula: ${formula}" >&2
  echo "create Formula/hekatessh.rb in ${tap_repo} from dist/brew/README.md first" >&2
  exit 1
fi
...
for target in "${assets[@]}"; do
  asset="hekatessh-v${version}-${target}.tar.gz"
  url="${base_url}/${asset}"
...
sha = ENV.fetch("SHA")
text = File.read(formula)
pattern = /(hekatessh-v#\{version\}-#{Regexp.escape(target)}\.tar\.gz"\n\s+)(?:# Fill after release\.\n\s+)?sha256 "[0-9a-f]{64}"/
...
if [ "${dry_run}" = "1" ]; then
  git diff -- Formula/hekatessh.rb
  exit 0
fi

git add Formula/hekatessh.rb
git commit -m "Update hekatessh ${tag} checksums"
git push -u origin "${branch}"
gh pr create \
  --repo "${tap_repo}" \
  --base main \
  --head "${branch}" \
  --title "Update hekatessh ${tag} checksums" \
