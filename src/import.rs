...
/// Read `~/.ssh/config`, resolve every host, and print an `ssh-mcp.toml`
/// skeleton to stdout.
pub fn run() -> Result<()> {
    ...
    for alias in aliases {
        match resolve_host(&alias) {
            ...
            Err(e) => eprintln!("ssh-mcp: skipping {alias:?}: {e:#}"),
        }
    }
...
/// Render the resolved hosts as an `ssh-mcp.toml` skeleton.
fn emit_toml(hosts: &[ResolvedHost]) -> String {
    let mut out = String::new();
    out.push_str("# Generated by ssh-mcp from ~/.ssh/config. Review before use:\n");
